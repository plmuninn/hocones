...
private[ops] class ExtractHoconValue[T <: SimpleHoconValue](
  ...
  private[ops] def extractValues: HoconResultValue => Iterable[T] = {
    ...
    case HoconEnvironmentValue(_, _, value) if isType(value) =>
      Iterable(value.asInstanceOf[T])
    case HoconValue(_, _, _, value) if isType(value) =>
      Iterable(value.asInstanceOf[T])
    case HoconReferenceValue(_, _, value) if isType(value) =>
      Iterable(value.asInstanceOf[T])
    case _ => Nil
  }
