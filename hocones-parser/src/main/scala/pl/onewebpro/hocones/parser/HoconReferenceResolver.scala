...
private[parser] object HoconReferenceResolver {
  ...
  private[parser] def resolveReferenceValueAsHoconEnvironmentValue(
    ...
    notResolvedReferenceToEnvironmentValue(hrv.result)
      .map(environmentValue =>
        HoconEnvironmentValue(hrv.path, hrv.cfg, environmentValue))

  private[parser] def resolveReferenceValue(
      resultList: FlatResultList,
      hrv: HoconReferenceValue): HoconResultValue =
    resultList
      .get(hrv.result.name)
  ...
  private[parser] def resolveResult(resultList: FlatResultList,
                                    value: Result): Result =
    value match {
      ...
      case composedConfigValue: ComposedConfigValue =>
        composedConfigValue.copy(
          values = composedConfigValue.values.map(configValue =>
            resolveSimpleValue(resultList, configValue)))
      case arr: HoconArray =>
      ...
      case concatenation: HoconConcatenation =>
        concatenation.copy(
          value = concatenation.value.copy(
            values = concatenation.value.values.map(value =>
              resolveSimpleValue(resultList, value))))
      case default => default
    }

  private[parser] def notResolvedReferenceToEnvironmentValue(
      notResolvedRef: NotResolvedRef): Option[EnvironmentValue] =
    notResolvedRef.nameChunks.lastOption
      .filter(name => name == name.toUpperCase)
      .map(_ => EnvironmentValue(notResolvedRef))

  private[parser] def resolveSimpleValue(
      resultList: FlatResultList,
      value: SimpleHoconValue): SimpleHoconValue =
    value match {
      case notResolvedRef: NotResolvedRef =>
        resultList
  ...
  def resolveReferences(resultList: FlatResultList,
                        values: Seq[HoconResultValue]): Seq[HoconResultValue] =
    values
      .map {
        case arr: HoconArray =>
          arr.copy(values = resolveReferences(resultList, arr.values))
        ...
        case hrv: HoconReferenceValue => resolveReferenceValue(resultList, hrv)
        case merged: HoconMergedValues =>
          merged.copy(defaultValue =
                        resolveResult(resultList, merged.defaultValue),
                      replacedValue =
                        resolveResult(resultList, merged.replacedValue))
        case concatenation: HoconConcatenation =>
          concatenation.copy(
            value = concatenation.value.copy(
              values = concatenation.value.values.map(value =>
  ...
  def apply(values: Iterable[HoconResultValue]): IO[Seq[HoconResultValue]] =
    for {
