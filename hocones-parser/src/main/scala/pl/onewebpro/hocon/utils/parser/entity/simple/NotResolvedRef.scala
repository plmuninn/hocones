...
import pl.onewebpro.hocon.utils.parser.entity.simple.NotResolvedRef.{HoconRefName, HoconRefValue}
...
object NotResolvedRef {
  ...
  private def tagEnvToRef(env: EnvValue): HoconRefValue = tag[HoconRefTag][String](env)

  private def tagEnvNameToRefName(env: EnvName): HoconRefName = tag[HoconRefNameTag][String](env)

  //TODO test me
  private[simple] def envIsRef(env: EnvValue): Boolean = env.contains(".")

  //TODO test me
  private[simple] def isRef(value: String): Boolean = EnvironmentValue.envName(value).exists(_.contains("."))

  //TODO test me
  def apply(value: String): IO[NotResolvedRef] =
    EnvironmentValue.envName(value).filter(envIsRef) match {
      case Some(env) => EnvironmentValue.extractName(env) match {
        case Some(name) =>
          IO(NotResolvedRef(env = tagEnvToRef(env), name = tagEnvNameToRefName(name), isOptional = EnvironmentValue.isOptionalEnv(env)))
        case None =>
