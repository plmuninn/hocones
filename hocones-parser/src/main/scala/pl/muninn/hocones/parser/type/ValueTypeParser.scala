...
import pl.muninn.hocones.parser.`type`.ValueType.ValueType
import pl.muninn.hocones.parser.entity._
import pl.muninn.hocones.parser.entity.simple.{ComposedConfigValue, EnvironmentValue, NotResolvedRef, SimpleHoconValue}
import pl.muninn.hocones.parser.{HoconParser, ParsingError}
...
object ValueTypeParser {
  ...
  private[parser] def divideToLeftAndRight(values: List[String]): IO[(String, String)] = {
    ...
    def getSingleValue(v: List[String]): IO[(String, List[String])] =
      for {
        startIndex <- IO(v.indexOf("["))
        result <- if (startIndex == -1) IO((v.mkString(""), Nil))
    ...
    value match {
      ...
      case ValueType.REFERENCE =>
        SimpleHoconValue(renderedValue).flatMap {
          case result: EnvironmentValue =>
            IO.pure(HoconEnvironmentValue(path, configValue, result))
