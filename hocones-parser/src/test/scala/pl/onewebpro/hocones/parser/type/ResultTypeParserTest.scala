...
import pl.onewebpro.hocones.parser.entity.{HoconArray, HoconObject, HoconValue}
...
class ResultTypeParserTest extends TestSpec {
  ...
  it should "parse ConfigList to HoconArray" in {
    ...
    val path = "pl.onewebpro.test.array.value_1"
    ...
    val expectedPaths = (0 to 4).map(i =>tagPath(s"pl.onewebpro.test.array.value_1.$i"))
  ...
  it should "parse ConfigObject to HoconObject" in {
    ...
    values.size shouldBe 5
    val expectedPaths = (1 to 5).map(i =>tagPath(s"pl.onewebpro.test.object.value_1.value_1_1_$i"))
    val expectedValueTypes = List(
      SimpleValueType.UNQUOTED_STRING,
      SimpleValueType.QUOTED_STRING,
      SimpleValueType.INT,
