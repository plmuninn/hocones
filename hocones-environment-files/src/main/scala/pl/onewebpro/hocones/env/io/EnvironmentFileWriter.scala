...
class EnvironmentFileWriter(file: OutputFile) {
  ...
  private[io] def environmentValueToString(
      value: EnvironmentValue): Iterable[String] =
    value.comments ++ Iterable(
      s"${value.name}=${value.defaultValue.getOrElse("")}\n")

  private[io] def composeEnvironmentValues(
      values: Iterable[EnvironmentValue]): SyncIO[Iterable[String]] =
    SyncIO(values.map(environmentValueToString).filter(_.nonEmpty).flatten)

  private[io] def writeValuesToFile(
      values: Iterable[String]): SyncIO[OutputFile] =
    Resource
      .fromAutoCloseable(SyncIO(new PrintWriter(file)))
      .use(printer => SyncIO(values.foreach(printer.println))) *> SyncIO.pure(
      file)
