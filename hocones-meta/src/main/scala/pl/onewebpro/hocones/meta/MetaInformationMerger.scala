...
object MetaInformationMerger {
  ...
  private def updateOrphans(
      loaded: MetaInformation,
      generated: MetaInformation): SyncIO[Seq[model.MetaValue]] =
    SyncIO
      .pure(generated.orphans)
      .map(_.map { value =>
        loaded.orphans
          .find(orphan =>
            orphan.name == value.name && orphan.getClass.getName == value.getClass.getName)
          .getOrElse(value)
      })

  def updateRoots(loaded: MetaInformation, generated: MetaInformation)
          ...
          case (innerKey, values) =>
            innerKey -> values.map { value =>
              loaded
                ...
                .filter(foundValue =>
                  foundValue.getClass.getName == value.getClass.getName)
                .getOrElse(value)
            }
        }
    })

  def merge(loaded: MetaInformation,
            generated: MetaInformation): SyncIO[MetaInformation] =
    for {
