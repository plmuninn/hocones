...
object Docs {
  ...
  case class DocsCommand(input: InputFile,
  ...
  object DocsCommand {
    def fromCommand(cliCommand: CliCommand): DocsCommand =
      DocsCommand(input = cliCommand.input,
                  output = None,
  ...
  val docsCommandF: Opts[DocsCommand] =
    (InputFile.opts,
     OutputFile.opts("documentation").orNone,
     TableAlignment.opts).mapN(DocsCommand.apply)

  val cmd: Opts[CliCommand] =
    Opts.subcommand("docs", "generate md table with environments")(docsCommandF)

  implicit private def mapCommandToConfig: DocsCommand => TableConfiguration = {
    command =>
      TableConfiguration(
  ...
  val docsCommand
    : Kleisli[IO, (HoconResult, MetaInformation, DocsCommand), Unit] = Kleisli {
    case (hocon, meta, command) =>
      for {
        tableConfiguration <- IO[TableConfiguration](command)
        _ <- putStrLn(
          Color.Green("Generating documentation about configuration"))
        _ <- MdGenerator
