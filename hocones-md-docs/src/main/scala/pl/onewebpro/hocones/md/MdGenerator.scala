...
object MdGenerator {
  ...
  def generateTable(result: HoconResult, meta: MetaInformation, config: TableConfiguration) = for {
  ...
  def generateDocument(result: HoconResult, meta: MetaInformation, config: DocumentConfiguration) = for {
    outputFile <- SyncIO(tagOutputFile(config.outputPath.toFile))
    parentDirectory <- SyncIO(tagParentDirectory(config.outputPath.getParent.toFile))
  ...
  def generate(result: HoconResult, meta: MetaInformation, configuration: Either[TableConfiguration, DocumentConfiguration]) =
    configuration match {
      case Left(tableConfiguration) => generateTable(result, meta, tableConfiguration)
      case Right(documentConfiguration) => generateDocument(result, meta, documentConfiguration)
    }
