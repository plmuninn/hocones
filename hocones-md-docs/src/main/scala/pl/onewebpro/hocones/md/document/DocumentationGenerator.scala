...
object DocumentationGenerator {
  ...
  private[document] def generateDocumentation(metaInformation: MetaInformation)(documents: List[Document[_]]): SyncIO[Documentation] = for {
    orphans <- SyncIO(documents.filter(_.path.isOrphan))
    roots <- SyncIO(metaInformation.roots.keys.map(root => root -> documents.filter(document => document.path.contains(root))).toMap)
  ...
  private[document] def mapToDocument(metaInformation: MetaInformation)(result: HoconResultValue): SyncIO[Document[_]] = {
    metaInformation.findByPathAndName(result.path) match {
      case Some(meta) => result match {
        case model: HoconArray => SyncIO.pure(ArrayDocument(model.path, meta, model))
        case model: HoconConcatenation => SyncIO.pure(ConcatenationDocument(model.path, meta, model))
        case model: HoconEnvironmentValue => SyncIO.pure(EnvironmentDocument(model.path, meta, model))
        case model: HoconMergedValues => SyncIO.pure(MergedValuesDocument(model.path, meta, model))
        case model: HoconObject => SyncIO.pure(ObjectDocument(model.path, meta, model))
        case model: HoconReferenceValue => SyncIO.pure(ReferenceValueDocument(model.path, meta, model))
        case model: HoconResolvedReference => SyncIO.pure(ResolvedReferenceDocument(model.path, meta, model))
        case model: HoconValue => SyncIO.pure(ValueDocument(model.path, meta, model))
      }
      case None => SyncIO.raiseError(MdFileError(s"No meta information for path ${result.path}"))
    }
  }

  def apply(result: HoconResult, meta: MetaInformation): SyncIO[DocumentationGenerator] =
    SyncIO.pure(result.results).flatMap { values =>
      values.map(mapToDocument(meta)).toList.sequence
    }.flatMap(generateDocumentation(meta)).map(documentation => new DocumentationGenerator(documentation))
