  ...
  implicit class MergedValuesBuilderOps(builder: TextBuilder) {
    ...
    def composeValue(result: Result): TextBuilder =
      result match {
        ...
        case merged: HoconMergedValues => builder.defaultValue(merged).replacedValue(merged)

        case reference: HoconReferenceValue => builder.referenceTo(reference)
        case resolvedReference: HoconResolvedReference => builder.referenceTo(resolvedReference)

        case environment: HoconEnvironmentValue => builder.name(environment).isOptional(environment)

        case value: HoconValue => builder.typeOfValue(value).valueOfDocument(value)
        ...
        case environmentValue: EnvironmentValue => builder.name(environmentValue).isOptional(environmentValue)

        case simpleValue: SimpleValue => builder.valueOfDocument(simpleValue)

        case notResolvedRef: NotResolvedRef => builder.referenceTo(notResolvedRef)

        case resolvedRef: ResolvedRef => builder.referenceTo(resolvedRef.reference)
      }
